namespace _3.FriendsOfPesho
{
    ...
    class Program
    {
        static void Main()
        {
            var graphParameters = Console.ReadLine().Split(' ');
            int nodesCount = int.Parse(graphParameters[0]);
            int pathsCount = int.Parse(graphParameters[1]);
            var hospitalNodes = Array.ConvertAll(Console.ReadLine().Split(' '), int.Parse);

            Graph graph = new Graph();

            for (int nodeName = 1; nodeName <= nodesCount; nodeName++)
            {
                graph.AddNode(nodeName);
            }

            foreach (int hospitalNode in hospitalNodes)
            {
                graph.Nodes[hospitalNode].IsHospital = true;
            }

            for (int path = 0; path < pathsCount; path++)
            {
                var pathParameters = Console.ReadLine().Split(' ');
                var fromNode = int.Parse(pathParameters[0]);
                var toNode = int.Parse(pathParameters[1]);
                var distance = int.Parse(pathParameters[2]);
            ...
            foreach (var hospital in hospitalNodes)
            {
                int currentDistance = 0;

                DijkstraAlgorithm(graph, graph.Nodes[hospital]);

                foreach (var home in graph.Nodes.Where(n => !n.Value.IsHospital).Select(h => h.Value))
                {
                    currentDistance += home.DijkstraDistance;
                }
        ...
        static void DijkstraAlgorithm(Graph graph, Node source)
        {
            ...
            var priorityQueue = new PriorityQueue<int, Node>();
            source.DijkstraDistance = 0;
            priorityQueue.Enqueue(source.Name, source);

            while (priorityQueue.Count != 0)
            {
                Node currentNode = priorityQueue.Dequeue().Value;
                ...
                foreach (var neighbourPath in currentNode.Connections)
                {
                    int potDistance = currentNode.DijkstraDistance + neighbourPath.Distance;

                    Node neighbour = neighbourPath.Target;
                    if (potDistance < neighbour.DijkstraDistance)
                    {
                        neighbour.DijkstraDistance = potDistance;
                        priorityQueue.Enqueue(neighbour.Name, neighbour);
                    }
                }
            }
        }
